...
import { locationsDirName, imagesDirName, uploadedDataDir } from "@/lib/dirPaths";
...
export async function POST(request: Request) {
    ...
    const seenLocationId = locationSchema.shape.id.parse(body["locationId"])
    ...
    const addedFileNamesPre = await Promise.all(
        Object.entries(body).map(async eachEntry => {
            ...
            if (eachEntryKey === "type" || eachEntryKey === "locationId") return null //skip type declaration

            const file = eachEntryValue as File;

            //ensure directory exists
            const mainDirectory = path.join(uploadedDataDir, locationsDirName, seenLocationId, imagesDirName)
